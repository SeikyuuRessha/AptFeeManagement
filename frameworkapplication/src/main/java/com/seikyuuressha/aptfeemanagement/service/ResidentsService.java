...
public class ResidentsService {
    ...
    public ResidentResponse getResident(String id) {
        return residentsMapper.toResidentResponse(residentsRepository.findById(id).orElseThrow(
                () -> new RuntimeException("Residents not found")
        ));
    }

    @PostAuthorize("returnObject.email == authentication.name")
    public ResidentResponse updateResident(String residentId, ResidentsCreationRequest request) {
        Residents residents = residentsRepository.findById(residentId).orElseThrow(
                () -> new RuntimeException("Residents not found")
        );
